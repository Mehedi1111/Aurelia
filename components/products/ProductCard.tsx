...
export default function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <article>
      {/*
        Single link wraps the entire card — fixes the "identical links" audit
        and gives a large tap target instead of two tiny overlapping ones.
      */}
      <Link
        href={href}
        className="group flex flex-col border border-border rounded-lg overflow-hidden hover:border-accent transition-colors duration-200"
      >
          ...
          {product.image ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-surface">
              <svg className="w-8 h-8 text-border" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M12 2l3 7h7l-5.5 4 2 7L12 16l-6.5 4 2-7L2 9h7z" />
              </svg>
            </div>
          )}
          {pct !== null && (
        ...
        <div className="flex flex-col flex-1 p-4">
          ...
          {/* Span styled as button — no second <a> needed since the card is already a link */}
          <span className="block text-center border border-dark text-dark text-xs font-medium py-2.5 rounded-md group-hover:bg-dark group-hover:text-white transition-colors">
            View Details
          </span>
        </div>
      </Link>
